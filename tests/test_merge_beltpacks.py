...
def test_merge_creates_in_pool():
    ...
    assert p5["role"] == "Régie Son" and p5["group_id"] is None
    assert "name" not in p5
    assert s["beltpack_roles"]["5"] == "Régie Son"


def test_merge_updates_role_preserves_group():
    s = model.empty_state()
    g = model.add_group(s, "Plateau", "#fff")
    p = model.add_person(s, "Ancien", "5", g["id"])
    res = model.merge_beltpacks(s, [{"number": "5", "name": "Régie Son", "online": True}])
    assert res == {"created": 0, "updated": 1}
    assert p["role"] == "Régie Son"        # nom mis à jour
    assert p["group_id"] == g["id"]        # groupe préservé


def test_merge_no_duplicate_and_no_delete():
    s = model.empty_state()
    model.add_person(s, "X", "9")   # fiche manuelle absente de l'antenne
...
def test_diff_reports_new_changed_unchanged_missing():
    s = model.empty_state()
    model.add_person(s, "Régie Son", "5")   # identique
    model.add_person(s, "Ancien", "7")      # rôle change
    model.add_person(s, "X", "9")           # absente de l'antenne
