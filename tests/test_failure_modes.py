...
def test_last_write_wins_two_admins(auth_client):
    ...
    p = auth_client.post("/api/people", json={"role": "HF", "beltpack": "12"}).get_json()
...
def test_duplicate_beltpack_blocked_on_patch(auth_client):
    ...
    p2 = auth_client.post("/api/people", json={"role": "Lum", "beltpack": "13"}).get_json()
...
def test_publish_then_display_only_sees_published(auth_client, app):
    ...
    auth_client.post("/api/people", json={"role": "HF", "beltpack": "99"})
